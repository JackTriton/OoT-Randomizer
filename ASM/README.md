...
### Running
- Install armips **0.11 or later** from <https://github.com/Kingcom/armips>. `src/build.asm` enforces this minimum version.
...
On Apple Silicon only, `build.py` also detects the matching Homebrew n64 prefix and creates a temporary isolated assembler wrapper under `ASM/build/macos-arm64-toolchain`. This prevents MacPorts or Conda environment entries from replacing the assembler selected by GCC. Set `OOTR_N64_PREFIX=/absolute/path/to/n64-prefix` to select a local installation explicitly.
### Running
...
`src/addresses.asm` is the single source of truth for `PAYLOAD_ROM_START`,
`PAYLOAD_START`, and `PAYLOAD_LIMIT`. `src/build.asm`, `src/boot.asm`, the C heap
guard, generated patch symbols, and Python runtime address conversion all use
those values. Branches that move the payload must update the related memory-map
constants in `src/addresses.asm` together so the payload, debug buffer, and C
heap do not overlap.
